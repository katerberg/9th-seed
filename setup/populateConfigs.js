...
const INSERT_CONFIG_TEMPLATE =
  'INSERT INTO configs (`config`, `stored`) VALUES ("{config}", "{stored}")';

function getInsert(config, value) {
  return INSERT_CONFIG_TEMPLATE.replace('{config}', config).replace(
    '{stored}',
    value
  );
}
...
connection.connectAsync().then(() => {
  ...
  const inserts = [
    ...
    getInsert(
      'sheet',
      'https://docs.google.com/spreadsheets/d/1yHAVTi7N8n42lvQirCX2j7VBgTUNADx4Lcfv-Aq027s/edit?usp=sharing'
    ),
    getInsert('challonge', 'https://challonge.com/stlotus10'),
  ];

  runScripts(inserts, 0)
    .catch((e) => {
